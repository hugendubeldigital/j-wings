/*
 * @(#)BeanIntrospector.java	1.2 00/08/25
 *
...
public class BeanIntrospector {
	...
	GenericBeanBoxInfo bi = (GenericBeanBoxInfo)beanInfoCache.get(beanClass);
	if (bi == null) {
	    bi = (new BeanIntrospector(beanClass, null, USE_ALL_BEANINFO)).getBeanInfo();
	    beanInfoCache.put(beanClass, bi);
	}

	// XXX - This is different from the java.beans.Introspector which returns 
	// a copy.
	return bi;
    }
	...
	GenericBeanBoxInfo bi = (new BeanIntrospector(beanClass, null, flags)).getBeanInfo();
	
	// Make an independent copy of the BeanInfo.
	return new GenericBeanBoxInfo(bi);
    }
	...
	throws IntrospectionException {
	GenericBeanBoxInfo bi = (new BeanIntrospector(beanClass, stopClass, USE_ALL_BEANINFO)).getBeanInfo();
    ...
    private BeanIntrospector(Class beanClass, Class stopClass, int flags)
	throws IntrospectionException {
	...
	if (superClass != stopClass) {
		...
		if (superBeanInfo == null) {
		    BeanIntrospector ins = new BeanIntrospector(superClass, null, USE_ALL_BEANINFO);
		    superBeanInfo = ins.getBeanInfo();
		    beanInfoCache.put(superClass, superBeanInfo);
		}
	    } else {
		BeanIntrospector ins = new BeanIntrospector(superClass, stopClass, newFlags);
	        superBeanInfo = ins.getBeanInfo();
	    }
	}
	if (informant != null) {
    ...
    private GenericBeanBoxInfo getBeanInfo() throws IntrospectionException {
	...
	MethodDescriptor mds[] = getTargetMethodInfo();
        ...
        return new GenericBeanBoxInfo(bd, esds, defaultEvent, pds,
	...
	if (old instanceof IndexedPropertyDescriptor ||
	    ...
	    composite = buildIndexedPropertyDescriptor(old, pd);
	} else {
	    composite = buildPropertyDescriptor(old, pd);
	}
	properties.put(name, composite);
    }
    
    static public PropertyDescriptor buildPropertyDescriptor(PropertyDescriptor old, PropertyDescriptor pd)  {
	Class[] ctypes = { old.getClass(), pd.getClass() };
	try {
	    Constructor cs = old.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { old, pd };
	    return (PropertyDescriptor)cs.newInstance(args);
    ...
    static public PropertyDescriptor buildIndexedPropertyDescriptor(PropertyDescriptor old, PropertyDescriptor pd)  {
	try {
	    IndexedPropertyDescriptor ipd = null;
	    if (old instanceof IndexedPropertyDescriptor) ipd = (IndexedPropertyDescriptor)old;
	    if (pd instanceof IndexedPropertyDescriptor) ipd = (IndexedPropertyDescriptor)pd;
	    Class[] ctypes = { ipd.getClass().getSuperclass(), ipd.getClass().getSuperclass() };
	    Constructor cs = ipd.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### idp: " + cs);
	    cs.setAccessible(true);
	    Object[] args = { old, pd };
	    return (PropertyDescriptor)cs.newInstance(args);
	} catch(NoSuchMethodException nsme) {
...
//	try {
//	    Method setter = (pd.getWriteMethod() != null) ? pd.getWriteMethod() : old.getWriteMethod();
//	    Method getter = (pd.getReadMethod() != null) ? pd.getReadMethod() : old.getReadMethod();
//	    Method indexedSetter = null;
//	    Method indexedGetter = null;
//	    if (old instanceof IndexedPropertyDescriptor) {
//		indexedGetter = ((IndexedPropertyDescriptor)old).getIndexedReadMethod();
//		indexedSetter = ((IndexedPropertyDescriptor)old).getIndexedWriteMethod();
//	    }
//	    if (pd instanceof IndexedPropertyDescriptor) {
//		Method get2 = ((IndexedPropertyDescriptor)pd).getIndexedReadMethod();
//		Method set2 = ((IndexedPropertyDescriptor)pd).getIndexedWriteMethod();
//		if (get2 != null) indexedGetter = get2;
//		if (set2 != null) indexedSetter = set2;
//	    }
//	    String gName = (getter != null) ? getter.getName() : null;
//	    String sName = (setter != null) ? setter.getName() : null;
//	    String igName =(indexedGetter != null) ? indexedGetter.getName() : null;
//	    String isName = (indexedSetter != null) ? indexedSetter.getName() : null;
//
//	    return new IndexedPropertyDescriptor(old.getName(), beanclass, gName, sName, igName, isName);
...
//	}
	...
	if (explicit != null) {
	    ...
	    while (keys.hasMoreElements()) {
	        ...
	        if (!BeanIntrospector.isSubclass(argType, eventListenerType)) {
		    continue;
	        }
	...
	EventSetDescriptor composite = buildEventSetDescriptor(old, esd);
	events.put(key, composite);
    }
    
    static public EventSetDescriptor buildEventSetDescriptor(EventSetDescriptor old, EventSetDescriptor esd) {
	Class[] ctypes = { old.getClass(), esd.getClass() };
	try {
	    Constructor cs = old.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { old, esd };
	    return (EventSetDescriptor)cs.newInstance(args);
	} catch(NoSuchMethodException nsme) {
	    nsme.printStackTrace();
	} catch(InvocationTargetException ite) {
	    ite.printStackTrace();
	...
	if (match) {
	    MethodDescriptor composite = buildMethodDescriptor(old, md);
    ...
    static private MethodDescriptor buildMethodDescriptor(MethodDescriptor old, MethodDescriptor md) {
	Class[] ctypes = { old.getClass(), md.getClass() };
	try {
	    Constructor cs = old.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { old, md };
	    return (MethodDescriptor)cs.newInstance(args);
	} catch(NoSuchMethodException nsme) {
	    nsme.printStackTrace();
	} catch(InvocationTargetException ite) {
	    ite.printStackTrace();
     ...
     * classloader, bootstrap classloader, then the class loader of the current Thread.
     */
	...
	// This code was added by Rene so that the introspector can get the
	// proper class loader.
...
class GenericBeanBoxInfo extends SimpleBeanInfo {
    
    public GenericBeanBoxInfo(BeanDescriptor beanDescriptor,
     ...
     */
    GenericBeanBoxInfo(GenericBeanBoxInfo old) {
	
	
	beanDescriptor = new BeanDescriptor(old.beanDescriptor.getBeanClass(), old.beanDescriptor.getCustomizerClass());
	if (old.events != null) {
	    int len = old.events.length;
	    events = new EventSetDescriptor[len];
	    for (int i = 0; i < len; i++) {
		events[i] = cloneEventSetDescriptor(old.events[i]);
	    }
	}
	...
	if (old.properties != null) {
		...
		if (oldp instanceof IndexedPropertyDescriptor) {
		    properties[i] = cloneIndexedPropertyDescriptor(
								      (IndexedPropertyDescriptor) oldp);
		} else {
		    properties[i] = clonePropertyDescriptor(oldp);
		}
	    }
	}
	...
	if (old.methods != null) {
	    ...
	    for (int i = 0; i < len; i++) {
		methods[i] = cloneMethodDescriptor(old.methods[i]);
	    }
	}
    ...
    public PropertyDescriptor clonePropertyDescriptor(PropertyDescriptor pd) {
	Class[] ctypes = { pd.getClass() };
	try {
	    Constructor cs = pd.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { pd };
	    return (PropertyDescriptor)cs.newInstance(args);
    ...
    public IndexedPropertyDescriptor cloneIndexedPropertyDescriptor(IndexedPropertyDescriptor pd) {
	Class[] ctypes = { pd.getClass() };
	try {
	    Constructor cs = pd.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { pd };
	    return (IndexedPropertyDescriptor)cs.newInstance(args);
	} catch(NoSuchMethodException nsme) {
	    nsme.printStackTrace();
	} catch(InvocationTargetException ite) {
	    ite.printStackTrace();
    ...
    public EventSetDescriptor cloneEventSetDescriptor(EventSetDescriptor ed) {
	Class[] ctypes = { ed.getClass() };
	try {
	    Constructor cs = ed.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { ed };
	    return (EventSetDescriptor)cs.newInstance(args);
    ...
    public MethodDescriptor cloneMethodDescriptor(MethodDescriptor md) {
	Class[] ctypes = { md.getClass() };
	try {
	    Constructor cs = md.getClass().getDeclaredConstructor(ctypes);
	    //System.out.println("### " + cs);
	    cs.setAccessible(true);
	    Object[] args = { md };
	    return (MethodDescriptor)cs.newInstance(args);
