...
public class BaseTableCG implements org.wings.plaf.BaseTableCG, SConstants
{
	...
	baseTable.add(new SCellRendererPane());
	installCellRenderer(baseTable);
    }
    public void uninstallCG(SComponent c) {
	SBaseTable baseTable = (SBaseTable)c;
	SCellRendererPane rendererComponent = getCellRendererPane(baseTable);
	baseTable.remove(rendererComponent);
    ...
    protected SCellRendererPane getCellRendererPane(SBaseTable baseTable) {
	int count = baseTable.getComponentCount();
	for (int i=0; i < count; i++) {
	    SComponent component = baseTable.getComponent(i);
	...
	SCellRendererPane rendererPane = getCellRendererPane(baseTable);
