...
public class ComboBoxCG implements org.wings.plaf.ComboBoxCG
{
	...
	comboBox.add(new SCellRendererPane());
	installCellRenderer(comboBox);
    }
    public void uninstallCG(SComponent component) {
	SComboBox comboBox = (SComboBox)component;
	SCellRendererPane rendererComponent = getCellRendererPane(comboBox);
	comboBox.remove(rendererComponent);
    ...
    protected SCellRendererPane getCellRendererPane(SComboBox comboBox) {
	int count = comboBox.getComponentCount();
	for (int i=0; i < count; i++) {
	    SComponent component = comboBox.getComponent(i);
	...
	if (model != null) {
	    SListCellRenderer cellRenderer = comboBox.getRenderer();
	    SCellRendererPane rendererPane = getCellRendererPane(comboBox);
